...
def create_one_dim_tr_model(
    ...
    :class:`deeptrade.models.OneDTransitionRewardModel` (see its documentation for explanation of some
    ...
    Returns:
        (:class:`deeptrade.models.OneDTransitionRewardModel`): the model created.
    ...
    if issubclass(hydra.utils._locate(model_cfg._target_), deeptrade.models.BasicEnsemble):
...
def create_replay_buffer(
    ...
    Returns:
        (:class:`deeptrade.replay_buffer.ReplayBuffer`): the replay buffer.
    """
    dataset_size = (
...
def get_basic_buffer_iterators(
    ...
    Args:
        replay_buffer (:class:`deeptrade.util.ReplayBuffer`): the replay buffer from which
    ...
    Returns:
        (tuple of :class:`deeptrade.replay_buffer.TransitionIterator`): the training
...
def get_sequence_buffer_iterator(
    ...
    Args:
        ...
        use_simple_sampler (int): if ``True``, returns an iterator of type
            :class:`deeptrade.replay_buffer.SequenceTransitionSampler` instead of
            :class:`deeptrade.replay_buffer.SequenceTransitionIterator`.
...
def train_model_and_save_model_and_data(
    model: deeptrade.models.Model,
    model_trainer: deeptrade.models.ModelTrainer,
    cfg: omegaconf.DictConfig,
    replay_buffer: ReplayBuffer,
    ...
    Args:
        model (:class:`deeptrade.models.Model`): the model to train.
        model_trainer (:class:`deeptrade.models.ModelTrainer`): the model trainer.
        ...
        replay_buffer (:class:`deeptrade.util.ReplayBuffer`): the replay buffer to use.
        ...
        callback (callable, optional): if provided, this function will be called after
            every training epoch. See :class:`deeptrade.models.ModelTrainer` for signature.
    """
    dataset_train, dataset_val = deeptrade.util.common.get_basic_buffer_iterators(
        replay_buffer,
...
def rollout_agent_trajectories(
    ...
    Args:
        ...
        replay_buffer (:class:`deeptrade.util.ReplayBuffer`, optional):
        ...
        agent_uses_low_dim_obs (bool): only valid if env is of type
            :class:`deeptrade.env.MujocoGymPixelWrapper` and replay_buffer is not ``None``.
...
def step_env_and_add_to_buffer(
    ...
    Args:
        ...
        callback (callable, optional): a function that will be called using the generated
            transition data `(obs, action. next_obs, reward, terminated, truncated)`.
        agent_uses_low_dim_obs (bool): only valid if env is of type
            :class:`deeptrade.env.MujocoGymPixelWrapper`. If ``True``, instead of passing the obs
            produced by env.reset/step to the agent, it will pass
            obs = env.get_last_low_dim_obs(). This is useful for rolling out an agent
            trained with low dimensional obs, but collect pixel obs in the replay buffer.
