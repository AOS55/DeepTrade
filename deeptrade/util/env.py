...
) -> Tuple[deeptrade.types.TermFnType, Optional[deeptrade.types.RewardFnType]]:
    import deeptrade.env

    term_fn = getattr(deeptrade.env.termination_fns, cfg.overrides.term_fn)
    if hasattr(cfg.overrides, "reward_fn") and cfg.overrides.reward_fn is not None:
        reward_fn = getattr(deeptrade.env.reward_fns, cfg.overrides.reward_fn)
    else:
        reward_fn = getattr(deeptrade.env.reward_fns, cfg.overrides.term_fn, None)
...
def _handle_learned_rewards_and_seed(
    cfg: Union[omegaconf.ListConfig, omegaconf.DictConfig],
    env: gym.Env,
    reward_fn: deeptrade.types.RewardFnType,
) -> Tuple[gym.Env, deeptrade.types.RewardFnType]:
...
def _legacy_make_env(
    cfg: Union[omegaconf.ListConfig, omegaconf.DictConfig],
) -> Tuple[gym.Env, deeptrade.types.TermFnType, Optional[deeptrade.types.RewardFnType]]:
    print(f"cfg.overrides.env: {cfg.overrides.env}")
    if cfg.overrides.env == "SingleInstrument-v0":
        env = deeptrade.env.instrument.SingleInstrumentEnv()
        term_fn = deeptrade.env.termination_fns.instrument
        reward_fn = deeptrade.env.reward_fns.instrument
...
class EnvHandler(ABC):
    ...
    def make_env(
        ...
        to have the following attributes (some are optional):
            ...
            :mod:`deeptrade.env.termination_fns`.
