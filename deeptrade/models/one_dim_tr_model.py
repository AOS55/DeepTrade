...
class OneDTransitionRewardModel(Model):
    ...
    To use with :class:deeptrade.models.ModelEnv`, the wrapped model must define methods
    ``reset_1d`` and ``sample_1d``.

    Args:
        model (:class:`deeptrade.model.Model`): the model to wrap.
    ...
    def __init__(
        ...
        self.model = model
        self.input_normalizer: Optional[deeptrade.util.math.Normalizer] = None
        if normalize:
            self.input_normalizer = deeptrade.util.math.Normalizer(
                self.model.in_size,
                self.model.device,
    ...
    def _get_model_input(
        self,
        obs: deeptrade.types.TensorType,
        action: deeptrade.types.TensorType,
    ) -> torch.Tensor:
    ...
    def update_normalizer(self, batch: deeptrade.types.TransitionBatch):
        ...
        Args:
            batch (:class:`deeptrade.types.TransitionBatch`): The batch of transition data.
    ...
    def loss(
        self,
        batch: deeptrade.types.TransitionBatch,
    ...
    def get_output_and_targets(
        self, batch: deeptrade.types.TransitionBatch
