...
def test_populate_replay_buffer_no_trajectories():
    num_steps = 100
    buffer = deeptrade.util.ReplayBuffer(1000, (1,), (1,), obs_type=int)
    env = MockEnv()
...
def test_populate_replay_buffer_collect_trajectories():
    num_trials = 10
    buffer = deeptrade.util.ReplayBuffer(
...
def test_get_basic_buffer_iterators():
    buffer = deeptrade.util.replay_buffer.ReplayBuffer(1000, (1,), (1,))
    dummy = np.ones(1)
    for i in range(900):
        buffer.add(dummy, dummy, dummy, i, False, False)

    train_iter, val_iter = deeptrade.util.common.get_basic_buffer_iterators(buffer, 32, 0.1)
...
def test_get_sequence_buffer_iterators():
    buffer = deeptrade.util.replay_buffer.ReplayBuffer(
    ...
    for sequence_length in range(1, 20):
        ...
        train_iter, val_iter = deeptrade.util.common.get_sequence_buffer_iterator(
...
def test_bootstrap_rb_sample_obs3d():
    ...
    obs_shape = (40, 40, 3)
    act_shape = (1,)
    buffer = deeptrade.util.ReplayBuffer(capacity, obs_shape, act_shape, obs_type=np.int8)
    ...
    it, _ = deeptrade.util.common.get_basic_buffer_iterators(
...
def test_truncated_normal():
    ...
    t_new = deeptrade.util.math.truncated_normal_(t_original)
